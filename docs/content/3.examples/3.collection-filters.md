...
Once you have a [collection page](/examples/collection-page), the next thing shoppers usually want is a way to narrow it
down.
...
This example builds a filter sidebar with a small component for each kind of filter, including a price range.

::card
  :::tabs
    ::::tabs-item{icon="i-lucide-eye" label="Preview"}
      :::::example-collection-filters{handle="men" .pt-2}
      :::::
    ::::
...
This example reuses the `ProductCard` component and the `PRODUCT_FRAGMENT` from the
[collection page example](/examples/collection-page), so it can stay focused on the filtering itself.
...
survives a refresh, the same way the [collection page](/examples/collection-page) keeps its pagination cursor there.
...
and because the selection lives in the URL, the query re-runs whenever it changes and the filtered view is shareable.

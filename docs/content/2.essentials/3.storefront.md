...
You can also wrap the `useStorefront` composable in another composable, to build an abstracted data fetching method:
...
Standard features of `useAsyncData` such as caching, revalidation, and error handling are all available when using `useStorefrontData`.
...
export default defineEventHandler((event) => {
  ...
  const storefront = useStorefront(event)
...
::note
Passing the `event` is optional, but it is recommended in order to forward the tracking headers of the incoming request.
When using a `privateAccessToken` and passing the event, the IP address of the buyer is sent through the 
`Shopify-Storefront-Buyer-IP` header, which Shopify uses to apply its rate limits per buyer instead of per server.
Set `buyerIp` on the client options to pass an address yourself, or `false` to not send the header.
::

...
::field-group
  ...
  Currency code (e.g. `USD`). Resolved from the Storefront API when omitted. Only the default market, see
  [Buyer market](#buyer-market) for multi-market shops.
  :::

  :::field{name="language" type="string"}
  Language code (e.g. `EN`). Resolved from the Storefront API when omitted. Only the default market, see
  [Buyer market](#buyer-market) for multi-market shops.
  :::
...
```ts [~/app/plugins/analytics-market.client.ts]
...
::field-group
  ...
  :::field{name="banner" type="boolean | object"}
  Load Shopify's own cookie banner. Defaults to `false`. Leave it off to build your own.
  :::

  :::field{name="banner.country" type="string"}
  Two-letter country code passed to Shopify's privacy banner.
  :::

  :::field{name="banner.language" type="string"}
  Locale passed to Shopify's privacy banner. Defaults to `analytics.language`.
  :::
::
...
If you set `banner: true`, Shopify renders its own banner and you can skip this component entirely.

### Reopening preferences

Once a visitor has made a choice, Shopify's banner disappears.
To reopen the consent dialog on demand, use the `showPreferences()` method from the composable:
...
<template>
  <button @click="analytics.showPreferences()">
    Cookie Preferences
  </button>
</template>
```

The dialog is Shopify's own, so this needs `banner` to be enabled.
It waits for the banner script to load before opening and Shopify only renders the banner in regions where consent applies.
When using your own banner, you have to handle the state yourself.

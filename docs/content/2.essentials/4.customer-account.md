...
[unstorage](https://unstorage.unjs.io) mount or driver (e.g. Redis) to keep them server-side instead, so they never
...
`routes.logout` instead, which performs the OpenID Connect logout.
::
...
::warning
A proxy is currently required to use the customer account client.
Direct calls to the Shopify API from the client side may land in the future, but are not supported at this time.
::

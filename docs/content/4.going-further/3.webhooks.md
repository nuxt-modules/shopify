...
::warning
HMAC validation only runs in production builds. In `nuxt dev`, every request is accepted without validation,
which makes local testing easier but means requests are not verified during development.
::

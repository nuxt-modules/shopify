...
Storefront types are auto-imported by default, while customer account and admin types are opt-in
(see [Auto-Imports](#auto-imports) below).
...
export default defineNuxtConfig({
  shopify: {
    clients: {
      ...
      customerAccount: {
        codegen: { autoImport: true }, // default: false (opt-in for customer account)
      },
      admin: {
        codegen: { autoImport: true }, // default: false (opt-in for admin)
      },
    },
    fragments: {
      dirs: ['graphql'],

---
title: Introduction
description: Welcome to the Nuxt Shopify documentation
navigation:
  icon: i-lucide-home
seo:
  title: Introduction
  description: Nuxt Shopify documentation introduction
---

Welcome to **Nuxt Shopify**, the Nuxt module integration for Shopify's e-commerce capabilities.

## Overview

Nuxt Shopify is designed to seamlessly integrate Shopify's powerful APIs into your Nuxt application.
To make development easier, the module automatically generates TypeScript types from your GraphQL operations,
hot-reloads on any changes made, auto-imports your GraphQL fragments, provides an easy, pre-configured
[mock.shop](https://mock.shop) integration, and [much more](#key-features).

No manual type definitions or complex codegen setup required.

::card-group
  :::card
  ---
  title: Storefront API
  icon: i-lucide-store
  to: /essentials/storefront
  ---
  Use the Storefront API to query your Shopify store
  :::

  :::card
  ---
  title: Customer Account API
  icon: i-lucide-user
  to: /essentials/customer-account
  ---
  Use the Customer Account API to manage your Shopify store's customer data
  :::

  :::card
  ---
  title: Admin API
  icon: i-lucide-shield
  to: /essentials/admin
  ---
  Use the Admin API to manage your Shopify store
  :::

  :::card
  ---
  title: Module configuration
  icon: i-lucide-settings
  to: /essentials/configuration
  ---
  Read up on the module configuration reference
  :::

  :::card
  ---
  title: Automatic type generation
  icon: i-lucide-wand-sparkles
  to: /going-further/codegen
  ---
  Generates types for your Shopify store automatically
  :::

  :::card
  ---
  title: Error Handling
  icon: i-lucide-bug
  to: /essentials/error-handling
  ---
  Learn how to handle errors in your application
  :::

  :::card
  ---
  title: Fragments
  icon: i-lucide-puzzle
  to: /getting-started/usage#fragments
  ---
  Learn how to manage GraphQL fragments in your application
  :::

  :::card
  ---
  title: Caching
  icon: i-lucide-rabbit
  to: /essentials/caching
  ---
  Cache API responses on the client and the server-side proxy
  :::

  :::card
  ---
  title: Analytics
  icon: i-lucide-chart-line
  to: /essentials/analytics
  ---
  Send Shopify analytics from your storefront with built-in consent handling
  :::

  :::card
  ---
  title: GraphQL Explorer
  icon: i-lucide-app-window
  to: /going-further/explorer
  ---
  Use a pre-configured explorer for testing
  :::

  :::card
  ---
  title: Webhooks
  icon: i-lucide-fishing-hook
  to: /going-further/webhooks
  ---
  Subscribe to Shopify webhooks and receive them with automatic validation
  :::

  :::card
  ---
  title: Hooks
  icon: i-lucide-between-horizontal-start
  to: /going-further/hooks
  ---
  Extend the module's functionality with hooks
  :::
::

## Key Features

- **TypeScript Support:** Creates fully typed fetch clients from your GraphQL schemas
- **Hot-reloading:** Hot-reloads types automatically when your GraphQL changes
- **Secure Access Token Handling:** Ensures your access tokens are stored securely
- **[Storefront](/essentials/storefront), [Customer Account](/essentials/customer-account) and [Admin](/essentials/admin) API Support:** Access all APIs with ease
- **Server & Client Side Support:** Works seamlessly on both server and client side
- **Automatic [mock.shop](https://mock.shop) Integration:** Easily integrate with mock.shop for testing
- **Built-in [Error Handling](/essentials/error-handling):** Optimized error handling for Nuxt applications
- **[Caching](/essentials/caching) Support:** Built-in caching on the server and client side for improved performance
- **[Analytics](/essentials/analytics) Support:** Send Shopify headless analytics with consent-aware tracking via the Customer Privacy API
- **Server-side Proxy:** Proxies API requests from the client through the Nitro server
- **GraphQL [Fragment](/getting-started/usage#fragments) Injection:** Reuse fragments across your queries, injected automatically and without duplicates
- **Customizable GraphQL [Code Generation](/going-further/codegen):** Tailor the code generation to your needs
- **[Auto-imports](/getting-started/usage#auto-imports):** Automatically imports your GraphQL fragments and generated types
- **[Webhooks](/going-further/webhooks) Support:** Easy subscription and validation for webhooks
- **Integrated [Explorer](/going-further/explorer):** Test your queries against every configured client, in dev only
- **Lifecycle [Hooks](/going-further/hooks):** Extend the module's functionality with ease

## Links

- [Shopify Storefront API](https://shopify.dev/docs/api/storefront)
- [Shopify Customer Account API](https://shopify.dev/docs/api/customer)
- [Shopify Admin API](https://shopify.dev/docs/api/admin-graphql)
- [GraphQL Codegen](https://the-guild.dev/graphql/codegen/docs/getting-started)
